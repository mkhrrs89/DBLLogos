(() => {
  ...
  async function buildNews() {
    ...
    try {
      ...
      generated.push(...await buildSingleGameNews(file, version, playerData.nameByPid));
      if (!isCurrent(file, version)) return;
  ...
  async function readPlayerHistories(file, version) {
    ...
    await stream.forEachTopLevelArrayItem(file, 'players', (player) => {
      ...
      if (pid !== null) nameByPid.set(pid, name);
  ...
  async function buildSingleGameNews(file, version, nameByPid) {
    const result = [];
    ...
    await stream.forEachTopLevelArrayItem(file, 'games', (game) => {
      ...
      for (const teamEntry of getGameTeams(game)) {
        ...
        for (const player of players) {
          const pid = readOptionalNumber(player?.pid);
          const name = getGamePlayerName(player, pid, nameByPid);
  ...
  function teamNameFor(tid, season) {
    if (tid === null || !Number.isFinite(season)) return '';
    try {
      if (typeof fullTimeline === 'undefined' || !Array.isArray(fullTimeline?.rows)) return '';
      ...
      return typeof entry?.teamName === 'string' ? entry.teamName : '';
    } catch (error) {
      return '';
    }
  }
